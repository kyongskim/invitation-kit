...
**진행 상태 (9주차 Day1~Day2 종료 시점):** `app/` (page·layout — `<html data-theme={config.theme}>` + Pretendard · Cormorant · Playfair Display · Italiana 4 폰트, `app/page.tsx` 에 `{config.guestbook.enabled && <Guestbook />}` 조건부 마운트), `app/globals.css` (`@theme` Classic 기본 + `:root[data-theme="modern"]` · `:root[data-theme="floral"]` override + `--radius-sm` 토큰), `app/fonts/` (Pretendard Variable), `components/sections/` (Main · Greeting · Gallery · Venue · Accounts · Guestbook · Share — Guestbook 은 orchestrator 단일 파일, 자식 둘은 `components/sections/guestbook/` 하위에 GuestbookForm · GuestbookList 분리), `components/` 최상위 (`DDayBadge` · `InAppBrowserNotice` — sections 외 Client 컴포넌트), `lib/` (`map.ts` 카카오맵·네이버 지도 · `calendar.ts` 구글 캘린더 · `kakao.ts` Kakao SDK wrapper · `clipboard.ts` · `date.ts` · `userAgent.ts` · `firebase.ts` Firestore db 싱글톤 · `hash.ts` bcryptjs 해싱 · `profanity.ts` 욕설 필터 (badwords-ko 574 + 자체 자음 변형 10) · `hooks.ts` `useIsClient` 추출), `public/images/og.png` · `public/images/gallery/sample-01~09.jpg`, `.env.example` 존재, `CHANGELOG.md` · `package.json` 메타 완성, `firestore.rules` · `firebase.json` 레포 루트. **`v0.2.0` 태그 + GitHub Release Latest 공개** (9주차 Day 1, 7주차 v0.1.0 패턴 미러). v0.2.0 이후 9 커밋 누적 — 가이드 3종 (`docs/{api-keys,config-guide,theme-guide}.md` 합계 812줄) + 9주차 회고 정정. ADR 총 6개. `.claude/rules/firebase.md` 9주차 강화 — 환경변수 섹션 + Console 8단계 + Gotcha 에 Vercel Environment Variables 등록 + Production·Preview 체크 + 재배포 단계 영구화 (v0.2 직후 dev 만 동작 / prod 작성 실패 사례), set-state-in-effect 메모 톤 격상 (절대 금지). `components/theme/`, `components/shared/` 여전히 미도입. `ThemeName = "classic" | "modern" | "floral"` 3종. **회고 우선 태스크 8주차→9주차 6/6 closure 달성** (v0.2 릴리스 + 가이드 3종 + 캘린더 검증 통과 + firebase.md 격상 + 회고 5번 가설 정정 + quality gate 시퀀스 명시).
