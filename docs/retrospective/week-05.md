...
- [ ] 태스크 5 — OG 이미지 제작·추가 **(6주차 이월; 디자인 자원 준비 블로커)**
...
1. **OG 이미지 제작·추가** (5주차 이월) — `public/images/og.jpg` 신규 (800×400 권장) + `app/layout.tsx` 의 `metadata.openGraph` 에 이미지·URL·title·description 추가. 현재 `share.thumbnailUrl` 이 `/images/og.jpg` 를 가리키는데 파일 미존재 상태라 카카오 공유 카드 썸네일이 빈 상태. 디자인 자원 준비 방향 (직접 · Figma 템플릿 · 이미지 생성 등) 은 사용자 결정.
2. **계좌번호 복사 섹션** — v0.1.0 Must 의 남은 핵심 기능. `components/sections/Accounts.tsx` 신규 + `lib/clipboard.ts` 헬퍼 (또는 바로 `navigator.clipboard` 사용) + `invitation.config.ts` 의 `accounts` 필드 실사용. 구현 난이도 낮음 (~1~2 시간).
3. **사진 갤러리** — 사용자 실제 청첩장 사진 5-10 장 준비 후 5-6 장면 분할 + lightbox UX. 난이도 중 (이미지 최적화 · 터치 제스처 · iOS Safari 검증 필요). 사진 자원 블로커.
4. **D-day 카운트다운** — 원래 v1.0 스코프였으나 난이도 낮아 MVP 로 당길 수도. `date` config 에서 Date 계산 후 Greeting 섹션 끝 또는 독립 섹션. 서버·클라이언트 시간 차이 주의 (SSR 시 Date.now() 는 빌드 시점 고정 → 클라이언트 hydration 후 재계산 필요).
5. **인앱 웹뷰 안내 UI** — 카카오톡 자체 웹뷰에서 청첩장 열면 `Kakao.Share.sendDefault` 실패 가능 (kakao-sdk.md "Gotcha" 기록). 현재는 폴백(URL 복사) 으로 자연 해소되지만 "카톡으로 받은 링크를 카톡에서 공유" 는 실사용 빈도 매우 높음. UA 분기로 안내 토스트 ("외부 브라우저에서 열어주세요") 띄우는 소규모 개선.
...
6. 다음 태스크 **한 개** 를 제안 — 6주차 첫 세션은 **태스크 1 (OG 이미지)** 가 자연스러우나 이미지 파일 준비 여부에 달림. 사용자가 "이미지 있음" 알리면 태스크 1, 아니면 **태스크 2 (계좌번호 복사)** 가 블로커 없이 바로 착수 가능
...
- **5주차 커밋 수**: 7 개 (이 회고 커밋 제외)
  - `80737ff` · `0c3717a` · `ef4bf68` · `20b215e` · `c3463c2` · `0864e02` · `22e90ad`
- **미푸시 커밋**: 0 개 (회고 커밋 전 기준)
- **CI 런**: 5 회 (3 커밋을 1 push 묶음 + 단일 push 4 회), 모두 녹색
- **외부 기여자**: 0 명
...
> **5주차는 "MVP 코드 인프라가 실제 프로덕션 환경에서 end-to-end 로 동작하는지" 를 처음 검증한 주간.** 카카오 콘솔 UI 개편과 `buttons[].link` 검증 정책이라는 두 외부 제약을 실기기 증상 → 번들 진단 → 공식 docs → 정책 확정 경로로 돌파해 `kakao-sdk.md` 에 영구 규칙으로 박았다. 네이버 지도 형제 함수 · Node 24 범프 · README 현실화까지 5주차 태스크 6 개 중 5 개 완료. MVP Must 에 남은 갤러리·계좌 복사 + OG 이미지 3 건은 6주차로. 12주 로드맵 문서화로 남은 7 주 재조정의 기준점도 세웠다.
