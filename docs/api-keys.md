...
### 1-3. JavaScript 키 + (선택) REST API 키 복사

생성된 앱 클릭 > 좌측 **앱 설정** > **앱 키** 탭. **JavaScript 키** 한 줄을 복사해 둡니다. **Admin · Native 키는 사용하지 않습니다** — 클라이언트 번들에 노출돼선 안 되는 키들입니다.

**REST API 키는 선택** — `npm run geocode` CLI (주소 → 좌표 자동 변환) 를 쓸 거면 같이 복사. 클라이언트에 노출 안 됨 (CLI 전용, `.env.local` 에만 두고 `KAKAO_REST_API_KEY` 환경 변수로 사용). 지도에서 직접 좌표 따낼 거면 REST 키 불필요.
...
### 2-7. (선택) App Check — 봇 스팸·스크래핑 방어

방명록·RSVP 가 인터넷에 노출되면 봇이 자동 발견·시도합니다. App Check 는 이를 reCAPTCHA v3 로 방어. **무료 티어 (1만 assessments/월) 안에서 청첩장 도메인엔 충분.** 도입 결정 근거는 [`docs/adr/009-app-check-strategy.md`](./adr/009-app-check-strategy.md).

1. Firebase Console 좌측 사이드바 > **빌드** > **App Check** > **시작하기**.
2. "내 앱" 섹션의 웹 앱 옆 **"App Check 등록"** > 공급자 **reCAPTCHA v3** 선택.
3. 안내 따라 **reCAPTCHA Admin Console** ([https://www.google.com/recaptcha/admin](https://www.google.com/recaptcha/admin)) 에서 site 등록 > 도메인에 본인 Vercel 도메인 (`*.vercel.app` 와 커스텀 도메인 모두) 추가 > **site key** 복사.
4. site key 를 `NEXT_PUBLIC_RECAPTCHA_SITE_KEY` 로 `.env.local` + Vercel 환경 변수 양쪽에 등록 + 재배포.
5. **enforcement 는 단계적**: App Check > APIs > Cloud Firestore > "Configure" > **"Monitor"** 또는 **"Unenforced"** 모드로 시작. 트래픽 누적 후 통과율 95%+ 확인되면 **"Enforce"** 토글. monitoring 단계 생략하면 정상 사용자가 차단되는 false positive 검증 기회 상실.
6. **로컬 dev**: App Check > Apps > 점 3개 > **"디버그 토큰 관리"** > 새 토큰 발급. dev 서버 첫 실행 시 브라우저 콘솔에 출력되는 token 을 Console 에 등록하면 reCAPTCHA 없이 통과.

**App Check 안 써도 됩니다** — `NEXT_PUBLIC_RECAPTCHA_SITE_KEY` 비워두면 init 자동 skip + 코드 그대로 동작 (방어 비활성). 일단 청첩장 운영 + 트래픽 보고 도입 결정해도 늦지 않음.
...
| `NEXT_PUBLIC_RECAPTCHA_SITE_KEY` (선택)    | App Check 등록 시 reCAPTCHA Admin Console   | `.env.local` + Vercel |
