...
방명록 구현 _전_ 에 규칙 파일 작성 — Progressive Disclosure 원칙 ("작업 시작 전에 룰 박아두기"). CLAUDE.md 의 placeholder 가 8주차에서 처음 실체화됨.
...
- **교훈**: 6주차 D-day 회고에 이미 한 번 나온 룰. 8주차 fetch 패턴에서 또 만난 건 **"룰을 알고 있다 ≠ 코드 짜는 손에 익었다"**. 다음 fetch effect 작성 시 _처음부터_ 핸들러로 빼는 습관. firebase.md 의 메모 톤도 "주의" → "fetch effect 는 초기 state 로 시작 상태 표현, 동기 setState 절대 금지" 로 격상하는 게 좋겠음 — Day4~5 또는 9주차 쓰기 시점에 갱신.
...
- **교훈**: firebase.md 의 "변형·confusable 정규화는 도입 안 함" 한 줄이 "사례 발생 시점에 ADR 로 격상" 의 트리거 메모 역할을 했음. 규칙 파일이 결정 _시점_ 까지 끌고 가는 도구로 작동 — 이 패턴 유지할 가치.
...
### 6. `ccc8c7a` 푸시 시점 CI format-check 실패 (fix-forward 로 복구)

자음 변형 보강 커밋 (`ccc8c7a`) 의 push 직후 CI 의 `format:check` job 이 ADR 006 (`docs/adr/006-profanity-filter-strategy.md`) 의 Prettier 미통과로 실패. 다음 분할 커밋 (`3b93f48`) 의 `git add -A` 가 ADR 006 의 prettier-formatted 변경도 함께 stage 해서 fix-forward 로 복구됨. main 의 현재 상태는 정상.

- **근본 원인**: 로컬 quality gate 시퀀스에 `format:check` 가 누락. lint · typecheck · build 만 실행하고 push. ADR 006 작성 후 한참 뒤 `npm run format` (write) 이 돌면서 ADR 006 도 자동 수정됐는데, 그 시점에 git stage 가 안 된 상태로 다른 변경에 묻혀 push 됨.
- **수정 방향**: 다음 작업부터 quality gate 시퀀스에 `format:check` 명시 — `lint && typecheck && format:check && build` 가 표준. `format` (write) 은 변경 후 즉시 `git add` 하는 습관 같이.
- **교훈**: `git add -A` 가 fix-forward 를 우연히 만들었지만, 그건 운. quality gate 누락은 push 전에 잡아야 함.

### 7. CI 폴링이 hook 정책에 막힘
...
5. **`gh run view` polling 허용을 .claude/settings.json 에 명시** — hook 차단으로 작업 흐름 끊긴 사례 후속. 5분 작업.
   - **9주차 fact-check (2026-04-25)**: 위 진단이 부정확. 글로벌 `~/.claude/settings.json` 은 빈 객체 (`{}`, hooks 0 개), 프로젝트 `.claude/settings.local.json` 에 `Bash(gh run *)` allow rule 이 이미 등록되어 있음. hook 차단이 아니라 하네스 레벨에서 long-running polling 자체를 일반 Bash 로 받지 않는 구조였음 — settings 변경으로 해결되는 문제가 아니다.
   - **채택한 패턴**: **Monitor 도구** (until-loop 으로 `gh run list` 폴링) — 9주차 Day 1 CI 검증 2회 모두 정상 동작. Bash 단발 호출 + `run_in_background` + `until` 루프도 동등한 fallback.
   - **9주차 Day 2 추가 finding**: Monitor 호출이 push-related 권한 룰로 거부되는 케이스 1 회 발생 (명령은 read-only `gh run list` 였음 — 메시지 패턴 매칭 오인 가능). 이 경우 즉시 Bash `run_in_background` + `until` 루프로 fallback, 동작 정상. 재발 시 패턴 메모리 저장 검토.
   - 결론: settings.json 명시 작업은 불필요. CI 폴링은 Monitor (1순위) → Bash `run_in_background` (fallback) 이 표준.

6. **Quality gate 시퀀스에 `format:check` 명시 포함** — `ccc8c7a` push 시점 CI 실패 (ADR 006 Prettier 미통과) 의 재발 방지. 로컬에서 `lint && typecheck && format:check && build` 가 표준 시퀀스. 작업 흐름 메모 1곳 수정.
...
- **CI 런**: 12 회. `24924424255` (자음 변형 보강) 1회 실패 — ADR 006 Prettier 미통과, 다음 커밋의 `git add -A` 로 fix-forward. 나머지 11 회 성공. React 19 룰 위반 1회 (방명록 fetch effect, 즉시 수정).
- **외부 기여자**: 0 명
