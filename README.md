...
- 📍 **오시는 길 (Venue)** — 주소 + 카카오맵·네이버 지도 딥링크 + **Google·Apple 캘린더 일정 추가** 버튼 (Apple 은 RFC 5545 + data URL 로 self-contained — 별도 호스팅 X) + 교통편 (지하철·버스·자가용·주차)
- 💰 **계좌번호 복사 (Accounts)** — 신랑/신부 세그먼트 토글 + 아코디언 + 하이픈 제거 복사 (일부 은행 앱 파싱 대응) + 카카오페이·토스 딥링크 (config 에 있을 때만 조건부)
- ✍️ **방명록 (Guestbook)** — Firebase Firestore + bcryptjs 비밀번호 해싱 (salt 10) + 욕설 필터 (badwords-ko 574 + 자체 자음 변형 10). 4상태 (loading/ready/error/empty) + optimistic prepend. 본인 삭제 (비밀번호 일치 검증, ADR 007 C')
- 📝 **RSVP** — 참석 여부 응답. 이름·참석여부·신랑/신부측·동반인원·메시지. 마감일 (`config.rsvp.deadline`) 후 form 자동 비활성. 응답은 Firestore `rsvp` 컬렉션 — read 차단 (host 가 Firebase Console 직접 조회). 자세한 결정 근거는 ADR 008
- 🎵 **배경 음악 (선택)** — 우상단 floating 토글. fade-in/out 300ms, loop, 자동재생 시도 X (iOS Safari 무음 모드 호환). 음원 파일은 OSS 라이선스 제약으로 ship 안 함 — 사용자가 본인 음원 (CC0 권장) 을 `public/audio/` 에 두고 `config.music.enabled = true`
- 🛡️ **App Check (선택)** — Firebase reCAPTCHA v3 통합. 방명록·RSVP 봇 스팸 방어. `NEXT_PUBLIC_RECAPTCHA_SITE_KEY` 등록 시 자동 활성, 미등록 시 graceful skip. enforcement 는 Firebase Console 토글로 monitoring → enforce 단계 전환. ADR 009
...
- 🌐 **OG 메타 태그** — 카카오톡·iMessage·Twitter 미리보기 썸네일 (800×400 `public/images/og.jpg`)
...
전체 후보·우선순위는 [`docs/00-roadmap.md`](./docs/00-roadmap.md#-v11-마일스톤) 참조. 누적 처리 항목: OG 최적화 ✅ · text-primary contrast ✅ · 방명록 본인 삭제 (C' 경로) ✅ · RSVP ✅ · BGM ✅ · App Check ✅ · Apple Calendar ✅. 미해결 후보: i18n · Pretendard dynamic-subset · 본인 삭제 서버 매개 (B 경로) 등
...
│   ├── calendar.ts           # Google · Apple 캘린더 URL pure function
