...
여기 솔직한 약점이 있다. **비밀번호 입력은 받지만 검증 경로가 없다.** v1.1 에서 A 경로 (Cloud Function) 를 도입할 때 그때 와서 받기 시작하면 그 이전 글은 본인 삭제가 영영 불가능하니 "미리 받아두자" 는 명목상 이유. 그런데 v1.1 이 안 오면 그 비밀번호는 영원히 의미 없는 60자 해시로 남는다. UI 가 약속한 것을 코드가 안 지키는, 약한 다크 패턴에 가깝다. 이건 12주차 또는 v1.1 첫 호흡에서 **(1) 비밀번호 입력 자체를 빼기 vs (2) Cloud Function 도입** 결정으로 닫힐 예정이다.
